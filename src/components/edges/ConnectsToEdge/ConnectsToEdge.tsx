...
import { useConnectsToEdge } from './useConnectsToEdge'
import type { ConnectsToEdgeProps } from './types'
...
const ConnectsToEdge = ({
  ...
  targetPosition,
  selected,
}: ConnectsToEdgeProps): React.JSX.Element => {
  const { isHovered, setIsHovered, handleDelete } = useConnectsToEdge(id)
  ...
  const markerEnd = 'url(#connects-to-arrow)'

  return (
    <>
      <defs>
        <marker
          id="connects-to-arrow"
          markerWidth="6"
          markerHeight="5"
          refX="5"
          refY="2.5"
          orient="auto"
        >
          <path d="M 0 0 L 6 2.5 L 0 5 z" fill="var(--node-db-accent)" />
        </marker>
      </defs>
