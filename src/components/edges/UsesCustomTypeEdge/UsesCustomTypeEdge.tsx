...
import { useUsesCustomTypeEdge } from './useUsesCustomTypeEdge'
import type { UsesCustomTypeEdgeProps } from './types'

const UsesCustomTypeEdge = ({
  id,
  sourceX,
  sourceY,
  targetX,
  ...
  targetPosition,
  selected,
}: UsesCustomTypeEdgeProps): React.JSX.Element => {
  const { isHovered, setIsHovered, handleDelete } = useUsesCustomTypeEdge(id)
  ...
  const markerEnd = 'url(#uses-custom-type-arrow)'

  return (
    <>
      <defs>
        <marker
          id="uses-custom-type-arrow"
          markerWidth="6"
          markerHeight="5"
          refX="5"
          refY="2.5"
...
export default UsesCustomTypeEdge
