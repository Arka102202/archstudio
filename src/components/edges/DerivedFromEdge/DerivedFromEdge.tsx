...
import { useDerivedFromEdge } from './useDerivedFromEdge'
import type { DerivedFromEdgeProps } from './types'

const DerivedFromEdge = ({
  id,
  sourceX,
  sourceY,
  targetX,
  ...
  targetPosition,
  selected,
}: DerivedFromEdgeProps): React.JSX.Element => {
  const { isHovered, setIsHovered, handleDelete } = useDerivedFromEdge(id)
  ...
  const markerEnd = 'url(#derived-from-arrow)'

  return (
    <>
      <defs>
        <marker
          id="derived-from-arrow"
          markerWidth="6"
          markerHeight="5"
          refX="5"
          refY="2.5"
...
export default DerivedFromEdge
