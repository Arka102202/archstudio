...
import { useStoredInEdge } from './useStoredInEdge'
import type { StoredInEdgeProps } from './types'
...
const StoredInEdge = ({
  ...
  targetPosition,
  selected,
}: StoredInEdgeProps): React.JSX.Element => {
  const { isHovered, setIsHovered, handleDelete } = useStoredInEdge(id)
  ...
  const markerEnd = 'url(#stored-in-arrow)'
