...
import { db } from '@db'
import type { InvokesEdgeHook } from './types'

// ─── useInvokesEdge ───────────────────────────────────────────────
// INVOKES is structural-only — deleting the edge requires no node reset.

export const useInvokesEdge = (edgeId: string): InvokesEdgeHook => {
