...
`ApplicationConfig.java` MUST always include a `Jackson2ObjectMapperBuilderCustomizer` bean that disables `WRITE_DATES_AS_TIMESTAMPS`. This ensures `LocalDate`, `LocalDateTime`, and all JSR-310 types serialize as ISO-8601 strings (e.g. `"2026-05-16"`) rather than arrays (e.g. `[2026, 5, 16]`):
...
`application.yml` MUST always include:
```yaml
spring:
  jackson:
    serialization:
...
```
This is non-negotiable — without it, any `LocalDate`/`LocalDateTime` field will serialize as a numeric array.
