# Chat — System Prompt & Action Format

> This file contains the system prompt for the canvas chat assistant.
> It is stack-agnostic in structure but Spring Boot specific in content.
> Used in src/utils/chatApi.ts as CHAT_SYSTEM_PROMPT.
> Import as a raw string: import CHAT_SYSTEM_PROMPT from '@/prompts/chatSystemPrompt.md?raw'
...
In `src/utils/chatApi.ts`, build the messages array as:

```typescript
const messages = [
  // Session summary (if exists) — injected as first user message with assistant ack
  ...(session.summary ? [
    { role: 'user',      content: `Previous session summary: ${session.summary}` },
  ...
  // Current user message — with optional architecture JSON
  {
    role: 'user',
    content: sendArchitecture
